...
fn format_colored() -> Format<ColorDecorator> {
    Format::new(
        FormatMode::Compact,
        ColorDecorator{use_color: true}
    )
...
fn format_plain() -> Format<ColorDecorator> {
    Format::new(
        FormatMode::Compact,
...
fn auto_format(is_atty : bool) -> Format<ColorDecorator> {
    if is_atty {
        format_colored()
    } else {
        format_plain()
...
pub fn stdout() -> Streamer<io::Stdout, Format<ColorDecorator>> {
    Streamer::new(
        io::stdout(),
        auto_format(stdout_isatty()),
    )
}
...
pub fn stderr() -> Streamer<io::Stderr, Format<ColorDecorator>> {
    Streamer::new(
        io::stderr(),
        auto_format(stderr_isatty())
    )
}
...
///
/// Automatically using color if printing to tty
pub fn async_stdout() -> AsyncStreamer<Format<ColorDecorator>> {
    AsyncStreamer::new(
        io::stdout(),
        auto_format(stdout_isatty()),
    )
...
///
/// Automatically using color if output goes to tty
pub fn async_stderr() -> AsyncStreamer<Format<ColorDecorator>> {
    AsyncStreamer::new(
        io::stderr(),
        auto_format(stderr_isatty()),
    )
